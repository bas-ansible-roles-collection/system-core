...
1. `test-basic` - Role is applied which will apply all dependent roles with their default variables
...
to bring up a local testing environment
2. `$ cd system-core/tests/provisioning/site-testing-local`
3. `$ vagrant up`
4. `$ cd ..`
...
1. `$ cd system-core/tests/provisioning`
...
Results for these tests can be found [here](https://semaphoreci.com/bas-ansible-roles-collection/system-core).
